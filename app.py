...
df_presence["Start DT"] = pd.to_datetime(df_presence["Start DT"], errors="coerce")
df_presence["End DT"] = pd.to_datetime(df_presence["End DT"], errors="coerce")
...
# --- Determine scheduled shift & presence for the day ---
...
if sched == "Not Assigned":
    ...
    # Shift scheduled + adherence data exists -> full dashboard
    ...
    # --- Shift Utilization (single % across shift) ---
    ...
    with col_aht1:
        st.metric("AHT Chat (mm:ss)", format_seconds_to_mm_ss(aht_chat) if aht_chat is not None else "N/A")
        st.metric("# Chat Items", num_chat_items)
    with col_aht2:
        st.metric("AHT Email (mm:ss)", format_seconds_to_mm_ss(aht_email) if aht_email is not None else "N/A")
        st.metric("# Email Items", num_email_items)

    # --- Shift Utilization Display ---
    st.markdown(f"""
    ...
    # Total Shift Duration = last end - first start
    if not agent_daily_presence_filtered.empty:
        ...
        total_shift_display = f"{hours:02d}:{minutes_int:02d}"
    else:
    ...
    st.markdown("---")
    # --- Lateness Statistics (30-day rolling) ---
    ...
    for d in window_days:
        ...
        shift_col_d = day_str
        sched_row_d = df_shifts[df_shifts["Agent Name"].str.lower() == agent.lower()]
        sched_val_d = sched_row_d[shift_col_d].values[0] if shift_col_d in df_shifts.columns and not sched_row_d.empty else None
